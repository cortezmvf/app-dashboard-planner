...
export function NewProjectDialog({ onClose, mode = 'project' }: Props) {
  ...
  const [name, setName] = useState(isTab ? `Dashboard ${(project?.tabs.length ?? 0) + 1}` : `Projeto ${store.projects.length + 1}`)
  ...
  function handleSubmit() {
    ...
    if (isTab) {
      ...
      if (conflict) { setError('Nome de dashboard já existe neste projeto'); return }
      store.createTab(trimmed, canvasW, canvasH)
    } else {
      const conflict = store.projects.some(p => p.name.toLowerCase() === trimmed.toLowerCase())
      if (conflict) { setError('Nome de projeto já existe'); return }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="font-semibold text-gray-900 dark:text-white">
            {isTab ? 'Novo Dashboard' : 'Novo Projeto'}
          </h2>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1 font-medium">
              {isTab ? 'Nome do dashboard' : 'Nome do projeto'}
            </label>
            <input
              autoFocus
              value={name}
              ...
              placeholder={isTab ? 'ex: Visão Geral' : 'ex: Analytics Q4'}
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-2 font-medium">Tamanho do canvas</label>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <div className="font-medium text-xs">Personalizado</div>
                <div className="text-[10px] text-gray-400 mt-0.5">Definir manualmente</div>
              </button>
            </div>
            ...
            {isCustom && (
              <div className="flex gap-2 mt-2">
                <div className="flex-1">
                  <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Largura (px)</label>
                ...
                <div className="flex-1">
                  <label className="text-xs text-gray-500 dark:text-gray-400 mb-1 block">Altura (px)</label>
        ...
        <div className="px-6 py-4 border-t border-gray-100 dark:border-gray-700 flex gap-2 justify-end">
          ...
          <button onClick={handleSubmit} className="px-4 py-2 text-sm bg-[#005175] hover:bg-[#003d58] text-white rounded-lg transition-colors font-medium">
            {isTab ? 'Criar Dashboard' : 'Criar Projeto'}
          </button>
        </div>
      </div>
    </div>
