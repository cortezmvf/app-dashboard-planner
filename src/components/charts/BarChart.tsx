...
export function BarChartPlaceholder({ chart, colors }: Props) {
  ...
  if (isHorizontal) {
    const pad = { t: chart.subtitle ? 38 : 24, r: 20, b: 8 + legendH, l: 50 }
    const W = chart.width
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
          ...
          return (
            <g key={i}>
              ...
              <text x={x} y={pad.t + cH + 10} fontSize={8} fill="#9ca3af" textAnchor="middle">{Math.round(pct * 100)}</text>
            </g>
          )
        })}
          ...
          if (isStacked) {
            ...
            return (
              <g key={i}>
                ...
                <text y={y + barH / 2 + 3} x={pad.l - 4} fontSize={8} fill="#9ca3af" textAnchor="end">{MONTHS[i]}</text>
              </g>
            )
          }
          return (
        ...
        if (isStacked) {
          ...
          return (
            <g key={i}>
              ...
              <text x={x + barW / 2} y={pad.t + cH + 12} fontSize={8} fill="#9ca3af" textAnchor="middle">{MONTHS[i]}</text>
            </g>
          )
        }
        return (
